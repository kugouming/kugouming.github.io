...
![brutal](../statics/images/fucking-algorithm/pictures/kmp/1.gif)

很明显，`pat` 中根本没有字符 c，根本没必要回退指针 `i`，暴力解法明显多做了很多不必要的操作。

KMP 算法的不同之处在于，它会花费空间来记录一些信息，在上述情况中就会显得很聪明：

![kmp1](../statics/images/fucking-algorithm/pictures/kmp/2.gif)

再比如类似的 txt = "aaaaaaab" pat = "aaab"，暴力解法还会和上面那个例子一样蠢蠢地回退指针 `i`，而 KMP 算法又会耍聪明：

![kmp2](../statics/images/fucking-algorithm/pictures/kmp/3.gif)
...
![](../statics/images/fucking-algorithm/pictures/kmp/txt1.jpg)

`dp` 数组指示 `pat` 这样移动：

![](../statics/images/fucking-algorithm/pictures/kmp/txt2.jpg)

PS：这个`j` 不要理解为索引，它的含义更准确地说应该是**状态**（state），所以它会出现这个奇怪的位置，后文会详述。

而对于 `txt2` 的下面这个即将出现的未匹配情况：

![](../statics/images/fucking-algorithm/pictures/kmp/txt3.jpg)

`dp` 数组指示 `pat` 这样移动：

![](../statics/images/fucking-algorithm/pictures/kmp/txt4.jpg)
...
![](../statics/images/fucking-algorithm/pictures/kmp/state.jpg)

如上图，圆圈内的数字就是状态，状态 0 是起始状态，状态 5（`pat.length`）是终止状态。开始匹配时 `pat` 处于起始状态，一旦转移到终止状态，就说明在 `txt` 中找到了 `pat`。比如说当前处于状态 2，就说明字符 "AB" 被匹配：

![](../statics/images/fucking-algorithm/pictures/kmp/state2.jpg)

另外，处于不同状态时，`pat` 状态转移的行为也不同。比如说假设现在匹配到了状态 4，如果遇到字符 A 就应该转移到状态 3，遇到字符 C 就应该转移到状态 5，如果遇到字符 B 就应该转移到状态 0：

![](../statics/images/fucking-algorithm/pictures/kmp/state4.jpg)

具体什么意思呢，我们来一个个举例看看。用变量 `j` 表示指向当前状态的指针，当前 `pat` 匹配到了状态 4：

![](../statics/images/fucking-algorithm/pictures/kmp/exp1.jpg)

如果遇到了字符 "A"，根据箭头指示，转移到状态 3 是最聪明的：

![](../statics/images/fucking-algorithm/pictures/kmp/exp3.jpg)

如果遇到了字符 "B"，根据箭头指示，只能转移到状态 0（一夜回到解放前）：

![](../statics/images/fucking-algorithm/pictures/kmp/exp5.jpg)

如果遇到了字符 "C"，根据箭头指示，应该转移到终止状态 5，这也就意味着匹配完成：

![](../statics/images/fucking-algorithm/pictures/kmp/exp7.jpg)


当然了，还可能遇到其他字符，比如 Z，但是显然应该转移到起始状态 0，因为 `pat` 中根本都没有字符 Z：

![](../statics/images/fucking-algorithm/pictures/kmp/z.jpg)

这里为了清晰起见，我们画状态图时就把其他字符转移到状态 0 的箭头省略，只画 `pat` 中出现的字符的状态转移：

![](../statics/images/fucking-algorithm/pictures/kmp/allstate.jpg)
...
![](../statics/images/fucking-algorithm/pictures/kmp/kmp.gif)
...
![](../statics/images/fucking-algorithm/pictures/kmp/forward.jpg)

**如果字符 `c` 和 `pat[j]` 不匹配的话**，状态就要回退（或者原地不动），我们不妨称这种情况为**状态重启**：

![](../statics/images/fucking-algorithm/pictures/kmp/back.jpg)

那么，如何得知在哪个状态重启呢？解答这个问题之前，我们再定义一个名字：**影子状态**（我编的名字），用变量 `X` 表示。**所谓影子状态，就是和当前状态具有相同的前缀**。比如下面这种情况：

![](../statics/images/fucking-algorithm/pictures/kmp/shadow.jpg)
...
![](../statics/images/fucking-algorithm/pictures/kmp/shadow1.jpg)
...
![](../statics/images/fucking-algorithm/pictures/kmp/A.gif)

当然，如果遇到的字符是 "B"，状态 `X` 也不能进行「状态推进」，只能回退，`j` 只要跟着 `X` 指引的方向回退就行了：

![](../statics/images/fucking-algorithm/pictures/kmp/shadow2.jpg)
...
![](../statics/images/fucking-algorithm/pictures/kmp/dfa.gif)
