...
var jdd = {
    ...
    compare: function () {
        ...
        var loadUrl = function (id, errId) {
            if (document.getElementById(id).value.trim().substring(0, 4).toLowerCase() === 'http') {
                ...
                document.getElementById('textarealeft').disabled  = true
                document.getElementById('textarearight').disabled = true
    ...
    sortJsonObject: function (obj) {
        if (Array.isArray(obj)) {  
            return obj.map(item => jdd.sortJsonObject(item)).sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b)));  
        } else if (typeof obj === 'object' && obj !== null) {  
            return Object.keys(obj).sort().reduce((result, key) => {  
                result[key] = jdd.sortJsonObject(obj[key]);  
                return result;  
            }, {});  
        }  
        return obj; // 对于基本类型，直接返回  
    },
    sortJsonString: function (str) {
        return JSON.stringify(jdd.sortJsonObject(JSON.parse(str)));
    }
};
...
document.addEventListener('DOMContentLoaded', function() {        
    ...
    document.getElementById('sortcompare').addEventListener('click', function () {
        // sort the json string before compare
        document.getElementById('textarealeft').value  = jdd.sortJsonString(document.getElementById('textarealeft').value);
        document.getElementById('textarearight').value = jdd.sortJsonString(document.getElementById('textarearight').value);
        jdd.compare();
    });
